        ...
        for glimpseI in range(n_glimpses):
            ...
            if keep:
                ...
                for h in range(glimpseI+1):
                    alpha += alpha_step
                    add_glimpses(axis=cur_ax[0], loc=cur_loc_norm[h,:], w=config.glimpse_size, n_patches=config.n_patches,
                                 color=color, linewidth=linewidth, alpha=alpha)

            else:
                add_glimpses(axis=cur_ax[0], loc=cur_loc_norm[glimpseI,:], w=config.glimpse_size, n_patches=config.n_patches, color=color, linewidth=linewidth)
...
def plot_trajectories(config=None, locations=[],
    ...
    if grid != []:
        assert grid[0]*grid[1] == N,   'Grid must have as many subplots as passed images. {} != {}.'.format(grid[0]*grid[1],N)
